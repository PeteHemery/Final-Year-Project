''********************************************
''*  VGA 320x240 2-Color Bitmap Driver v1.02 *
''*  (C) 2006, 2007 Parallax, Inc.           *
''********************************************
''
'' This object generates a 320x240 pixel bitmap, signaled as 640x480 VGA.
'' Each pixel is one bit, so the entire bitmap requires 320 x 240 / 32 longs,
'' or 2,400 longs (9.6KB). Color words comprised of two byte fields provide
'' unique colors for every 32x16 pixel group. These color words require 320/32
'' * 240/16 words, or 150 words. Pixel memory and color memory are arranged
...
''
'' I needed a low-res VGA (QVGA really) graphics driver 
'' so I modified the 512x384 driver :)
''
'' William Henning
...
' for my Innovatek tiny monitor, change vf to 4, and vb to 33 to perfectly center it
  ...
  hf = 12       'horizontal front porch pixels org 40
  ...
  hn = 1        'horizontal normal sync state (0|1)
  vn = 1        'vertical normal sync state (0|1)
  pr = 15       'pixel rate in MHz at 80MHz system clock (5MHz granularity)
...
''
''    ColorPtr = Pointer to 150 words which define the "0" and "1" colors for
...
''
''    PixelPtr = Pointer to 2,400 longs containing pixels that make up the 320 x
